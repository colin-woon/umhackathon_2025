...
# --- Performance Metrics (Handles Daily/Hourly Timeframes) ---
def calculate_performance(df_backtest, fee_percent, timeframe='daily', start_value=1.0): # Added timeframe argument
    ...
    # This assumes starting value is 1.0 for return calculations if portfolio value isn't pre-calculated
    ...
    if 'strategy_return_net' in df_backtest.columns:
        ...
        # Annualized return = mean_periodic_return * periods_per_year
        # Annualized std dev = std_dev_periodic_return * sqrt(periods_per_year)
        if std_dev_periodic_return != 0 and periods_per_year > 0:
             ...
             # Alternative: (mean_periodic_return * periods_per_year) / (std_dev_periodic_return * np.sqrt(periods_per_year))
        else:
             sharpe_ratio = 0
